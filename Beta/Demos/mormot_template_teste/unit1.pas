...
procedure TForm1.btnrecarregarClick(Sender: TObject);
var
  sJSON: string;
begin
  // Pega o texto do Memo e remove quebras de linha
  sJSON := Memo1.Text;
  sJSON := StringReplace(sJSON, #13#10, '', [rfReplaceAll]); // Remove CR+LF
  sJSON := StringReplace(sJSON, #13, '', [rfReplaceAll]);     // Remove CR sozinho
  sJSON := StringReplace(sJSON, #10, '', [rfReplaceAll]);     // Remove LF sozinho


 Session.ExecJS('PopulaDados(' +  sJSON  + ')');
end;
...
begin
  ...
  D2Bridge.FrameworkExportType.TemplateMasterHTMLFile:= 'index.html';
...
var
  teste: string;
begin
  if SameText('GridEdit', CallBackName) then
  begin
   teste := EventParams.Values['RecNo'];
   ShowMessage('GridEdit ID: ' + teste);
  end;

  if SameText('GridDelete', CallBackName) then
  begin
   teste := EventParams.Values['RecNo'];
   ShowMessage('GridDelete ID: ' + teste);
  end;
end;
