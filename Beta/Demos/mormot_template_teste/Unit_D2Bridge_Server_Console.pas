...
begin
 D2BridgeServerController:= Ttemplate_testeWebAppGlobal.Create(nil);
 ...
 vServerName:= D2BridgeServerController.APPConfig.ServerName('template_teste');

 D2BridgeServerController.APPName:= 'template_teste';
...
// * REST OPTIONS
{
 ...
 begin
  // SSL cert files — configure as needed. Use local var compatible with both record (mORMot2) and class (Indy) return types
   {$IFNDEF USE_MORMOT2}
   with D2BridgeServerController.Prism.SSLOptions do
   begin
     CertFile:= '';
     KeyFile:= '';
     RootCertFile:= '';
