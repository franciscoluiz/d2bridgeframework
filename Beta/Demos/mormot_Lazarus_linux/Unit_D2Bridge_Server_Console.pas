...
begin
 ...
 D2BridgeServerController.Languages:= [TD2BridgeLang.English];
  ...
  begin
   // SSL config: set via local copy (record type in mORMot2)
{$IFNDEF USE_MORMOT2}
...
{$ENDIF}
 ...
 D2BridgeServerController.Prism.Options.PathCSS:= 'css';
  ...
  // SSL Support — enable with SSL=1 and SSL Port=4443 in Config.ini [D2Bridge Server Config]
  if D2BridgeServerController.APPConfig.FileINIConfig.ReadInteger('D2Bridge Server Config', 'SSL', 0) = 1 then
  begin
    D2BridgeServerController.Prism.Options.SSL := true;
    vServerPort := D2BridgeServerController.APPConfig.FileINIConfig.ReadInteger('D2Bridge Server Config', 'SSL Port', 4443);
  end;
