...
uses
  LazarusWebApp, D2BridgeFormTemplate, D2Bridge.API.Mail;
...
begin
  ...
  // Teste de envio de e-mail via mORMot2 SMTP
  vMail := TD2BridgeAPIMail.Create;
  try
    // 0d10ab8e03be8d699f50abc1ea032411
    vMail.Config.Host := 'sandbox.smtp.mailtrap.io';
    vMail.Config.Port := 2525;
    vMail.Config.UserName := 'babf4bf3dc8259';
    vMail.Config.Password := '3f54303ac49135';
    vMail.Config.UseTLS := False;
    vMail.Config.UseSSL := False;
    with vMail.From do
    begin
      Name := 'D2Bridge Test';
      MailAddress := 'from@example.com';
    end;
    with vMail.Adresses.Add do
    begin
      Name := 'Destinatario';
      MailAddress := 'franciscoluiz@gmail.com';
    end;
    vMail.Subject := 'D2Bridge mORMot2 SMTP Test';
    vMail.BodyHTML.Text := '<h1>Teste de envio de e-mail</h1><p>Se voce esta vendo isso, o SMTP via mORMot2 funcionou!</p>';
    if vMail.SendMail then
      ShowMessage('Email enviado com sucesso!')
    else
      ShowMessage('Falha ao enviar e-mail. Verifique credenciais SMTP no codigo (Button2Click).');
  finally
    vMail.Free;
  end;
end;
